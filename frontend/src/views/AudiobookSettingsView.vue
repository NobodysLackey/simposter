...
const mappingFor = (libraryId: string) =>
  settings.value.library_mappings.find((mapping) => String(mapping.id) === String(libraryId))

const ensureMapping = (library: PlexLibrary) => {
  let mapping = mappingFor(library.id)
  if (!mapping) {
    mapping = {
      id: library.id,
      title: library.title,
...
const toggleLibrary = (library: PlexLibrary, enabled: boolean) => {
  const mapping = ensureMapping(library)
        ...
        <div v-if="libraries.length" class="library-grid">
          <article v-for="library in libraries" :key="library.id" class="library-card">
            <label class="library-toggle">
              <input
                type="checkbox"
                :checked="mappingFor(library.id)?.enabled || false"
                @change="toggleLibrary(library, ($event.target as HTMLInputElement).checked)"
              />
              <strong>{{ library.title }}</strong>
              <span>Library {{ library.id }}</span>
            </label>

            <template v-if="mappingFor(library.id)?.enabled">
              <label class="field-label">
                Display Name
                <input v-model="ensureMapping(library).display_name" type="text" />
              </label>
              <label class="field-label">
                Default Preset
                <select v-model="ensureMapping(library).default_preset_id">
