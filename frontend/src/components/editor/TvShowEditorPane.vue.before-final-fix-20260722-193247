...
const posters = ref<
  { url: string; thumb?: string; has_text?: boolean; language?: string; source?: string }[]
>([])
const logos = ref<
  {
    url: string
...
const showLevelAssets = ref<{ posters: any[]; logos: any[]; backdrops?: any[] }>({
  posters: [],
  logos: [],
  backdrops: [],
})
...
async function switchToRenderedPreview(index: number) {
  ...
  if (preview) {
    ...
    const seasonIndex = Array.from(selectedSeasons.value).findIndex(
      (key) => key === preview.seasonKey,
    )
    if (seasonIndex >= 0) {
...
const loadEditorState = () => {
  try {
    ...
    if (state.posterLanguageFilter && state.posterLanguageFilter !== 'all')
      posterLanguageFilter.value = state.posterLanguageFilter
    if (typeof state.showTmdbPosters === 'boolean') showTmdbPosters.value = state.showTmdbPosters
    if (typeof state.showFanartPosters === 'boolean')
      showFanartPosters.value = state.showFanartPosters
    ...
    // Only restore language filters if they're not set to default 'all' (prefer global settings)
    if (state.logoLanguageFilter && state.logoLanguageFilter !== 'all')
      logoLanguageFilter.value = state.logoLanguageFilter
...
const filteredLogos = computed(() => {
  ...
  const scoreLogo = (l: any) => {
    ...
    const isWhite =
      url.includes('white') || url.includes('_w') || url.includes('-w') || url.includes('light')
...
const optionsPayload = computed(() => {
  ...
  const payload: Record<string, any> = {
    ...
    overlay_config_ids:
      options.value.overlayConfigIds.length > 0 ? options.value.overlayConfigIds : undefined,
  }
  ...
  if (posterFilter.value === 'textless') {
    ...
    if (choice) {
      ...
      if (!selectedPoster.value) selectedPoster.value = choice.url
        ...
        const currentIsTextless = [...seasonPosters, ...seriesPosters].some(
          (p) => p.url === selectedPoster.value && p.has_text === false,
        )
...
const fetchImagesForSeason = async (season: Season) => {
  ...
  try {
    ...
    const imgRes = await fetch(
      `${apiBase}/api/tmdb/tv/${tmdbId.value}/images?season=${season.index}${tvdbQuery}`,
    )
...
const fetchImagesForCurrentSeason = async (skipRestore = false) => {
  ...
  if (assets) {
    ...
    if (!season.isSeries && posterFilter.value === 'textless') {
      const seriesTextlessPosters = (showLevelAssets.value.posters || []).filter(
        (p: any) => p.has_text === false,
      )
...
const fetchLabels = async () => {
  try {
    ...
    selectedLabels.value = new Set(
      defaultToRemove.filter((label: string) => labels.value.includes(label)),
    )
  } catch (e) {
    console.error(e)
  }
...
const fetchExistingPoster = async (forceRefresh?: boolean | Event) => {
  try {
    ...
    const res = await fetch(
      `${apiBase}/api/movie/${targetKey}/poster?meta=1${refreshFlag ? '&force_refresh=1' : ''}`,
    )
    if (!res.ok) {
    ...
    if (data.url) {
      ...
      existingPoster.value = data.url.startsWith('http') ? data.url : `${apiBase}${data.url}`
      ...
      seasons.value = seasons.value.map((s) =>
        s.key === targetKey ? { ...s, thumb: existingPoster.value || s.thumb } : s,
      )
    } else {
      existingPoster.value = null
      updateGlobalPosterCache(targetKey, null)
...
const doPreview = async (skipBackgroundRender = false) => {
  ...
  const cacheKey = season
    ? `${season.key}_${capturedBgUrl}_${capturedLogoUrl}_${JSON.stringify(capturedOptionsPayload)}`
    : ''
  ...
  const renderPromises = seasonsToRender.map(async (season) => {
    ...
    if (!cachedPoster) {
      ...
      if (posterFilter.value === 'textless') {
        ...
        const seriesTextless = isSeason
          ? seriesPosters.find((p: any) => p.has_text === false)
          : null
        const choice = seasonTextless || seriesTextless
    ...
    const hasUserModifications =
      cachedSettings && (userModifiedFields.value[seasonKey]?.size ?? 0) > 0
...
watch(seriesPosterUrl, (url) => {
  ...
  if (idx === -1) return
  seasons.value = seasons.value.map((s, i) => (i === idx ? { ...s, thumb: url || s.thumb } : s))
})
...
function prevSeason() {
  ...
  currentSeasonIndex.value =
    currentSeasonIndex.value === 0 ? selectedCount - 1 : currentSeasonIndex.value - 1
}
...
onMounted(async () => {
  ...
  fetch(`${apiBase}/api/fonts`)
    ...
    .then((d) => {
      if (d?.fonts) availableFonts.value = d.fonts
    })
    .catch(() => {})
})
...
const applyPresetOptions = (id: string, opts: PresetApplyOptions = {}) => {
  ...
  if (typeof o.uniform_logo_offset_x === 'number')
    options.value.uniformLogoOffsetX = Math.round(o.uniform_logo_offset_x * 100)
  if (typeof o.uniform_logo_offset_y === 'number')
    options.value.uniformLogoOffsetY = Math.round(o.uniform_logo_offset_y * 100)
  if (typeof o.uniform_logo_h_align === 'string')
    options.value.uniformLogoHAlign = o.uniform_logo_h_align as 'left' | 'center' | 'right'
  if (typeof o.uniform_logo_v_align === 'string')
    options.value.uniformLogoVAlign = o.uniform_logo_v_align as 'top' | 'center' | 'bottom'
  ...
  if (typeof o.overlay_opacity === 'number')
    options.value.overlayOpacity = Math.round(o.overlay_opacity * 100)
  ...
  if (
    typeof o.poster_filter === 'string' &&
    ['all', 'textless', 'text'].includes(o.poster_filter)
  ) {
    posterFilter.value = o.poster_filter as 'all' | 'textless' | 'text'
  }
  if (
    typeof o.logo_preference === 'string' &&
    ['first', 'white', 'color'].includes(o.logo_preference)
  ) {
...
watch(
  ...
  () => markFieldModified('posterZoom'),
)
watch(
  () => options.value.posterShiftY,
  () => markFieldModified('posterShiftY'),
)
watch(
  () => options.value.matteHeight,
  () => markFieldModified('matteHeight'),
)
watch(
  () => options.value.fadeHeight,
  () => markFieldModified('fadeHeight'),
)
watch(
  () => options.value.vignette,
  () => markFieldModified('vignette'),
)
watch(
  () => options.value.grain,
  () => markFieldModified('grain'),
)
watch(
  () => options.value.logoScale,
  () => markFieldModified('logoScale'),
)
watch(
  () => options.value.logoOffset,
  () => markFieldModified('logoOffset'),
)
      ...
      <div class="pane-header">
        <div>
          ...
          <h2>
            ...
            <span v-if="selectedPosterType === 'season'" class="poster-type-badge season-badge"
              >Season Poster</span
            >
      ...
      <div class="controls-scroll">
        ...
        <div class="acc-section">
          <button class="acc-header" @click="toggleSection('template')">
            ...
            <svg
              class="acc-chevron"
              :class="{ open: sectionOpen.template }"
              width="14"
              height="14"
              viewBox="0 0 24 24"
              fill="none"
          ...
          <div v-show="sectionOpen.template" class="acc-body">
            <div class="preset-row">
              ...
              <button
                ...
                title="Save current settings to preset"
              >
                <svg
                  width="16"
                  height="16"
            ...
            <div class="preset-row new-preset-row">
              <input
                v-model="newPresetId"
                type="text"
                placeholder="New preset id"
                class="new-preset-input"
              />
              <button
                class="save-preset-btn wide"
        ...
        <div class="acc-section">
          ...
          <div v-show="sectionOpen.poster" class="acc-body">
            ...
            <div class="poster-counts">
              TMDb: {{ posterCounts.tmdb }} · Fanart: {{ posterCounts.fanart }} · TVDB:
              {{ posterCounts.tvdb }}
            </div>
            ...
            >
              <template v-if="uploadedPosterUrl">
                ...
                <div class="upload-overlay">
                  <button
                    class="upload-reselect"
                    @click.stop="selectedPoster = uploadedPosterUrl"
                    :class="{ active: selectedPoster === uploadedPosterUrl }"
                  >
                    Use this
                  </button>
                  <button
                    class="upload-replace"
                    @click.stop="($refs.posterFileInput as HTMLInputElement)?.click()"
                  >
                    Replace
                  </button>
            ...
            <div class="slider">
              ...
              <div class="slider-row">
                ...
                <input
                  v-model.number="options.posterShiftY"
                  type="number"
                  min="-50"
                  max="50"
                  class="slider-num"
            ...
            <div class="slider">
              ...
              <div class="slider-row">
                ...
                <input
                  v-model.number="options.matteHeight"
                  type="number"
                  min="0"
                  max="50"
                  class="slider-num"
            ...
            <div class="slider">
              ...
              <div class="slider-row">
                ...
                <input
                  v-model.number="options.fadeHeight"
                  type="number"
                  min="0"
                  max="100"
                  class="slider-num"
            ...
            <div class="slider">
              ...
              <div class="slider-row">
                ...
                <input
                  v-model.number="options.vignette"
                  type="number"
                  min="0"
                  max="100"
                  class="slider-num"
        ...
        <div class="acc-section">
          ...
          <div v-show="sectionOpen.logo" class="acc-body">
            <label
              v-if="isUniformLogo && !isLogoNone"
              class="inline-field checkbox"
            ...
            <template v-if="!isLogoNone">
              ...
              <div class="poster-counts">
                TMDb: {{ logoCounts.tmdb }} · Fanart: {{ logoCounts.fanart }} · TVDB:
                {{ logoCounts.tvdb }}
              </div>
              ...
              <template v-if="!isUniformLogo">
                ...
                <div class="slider">
                  ...
                  <div class="slider-row">
                    ...
                    <input
                      v-model.number="options.logoOffset"
                      type="number"
                      min="0"
                      max="100"
                      class="slider-num"
              ...
              <template v-if="isUniformLogo">
                <div class="slider">
                  ...
                  <div class="slider-row">
                    <input
                      v-model.number="options.uniformLogoMaxW"
                      type="range"
                      min="50"
                      max="1800"
                    />
                ...
                <div class="slider">
                  ...
                  <div class="slider-row">
                    <input
                      v-model.number="options.uniformLogoMaxH"
                      type="range"
                      min="50"
                      max="2800"
                    />
                ...
                <div class="slider">
                  ...
                  <div class="slider-row">
                    <input
                      v-model.number="options.uniformLogoOffsetX"
                      type="range"
                      min="0"
                      max="100"
                    />
                ...
                <div class="slider">
                  ...
                  <div class="slider-row">
                    <input
                      v-model.number="options.uniformLogoOffsetY"
                      type="range"
                      min="0"
                      max="100"
                    />
        ...
        <div class="acc-section">
          <button class="acc-header" @click="toggleSection('overlay')">
            ...
            <svg
              class="acc-chevron"
              :class="{ open: sectionOpen.overlay }"
              width="14"
              height="14"
              viewBox="0 0 24 24"
              fill="none"
          ...
          <div v-show="sectionOpen.overlay" class="acc-body">
            ...
            <div v-if="options.borderEnabled" class="slider">
              ...
              <div class="slider-row">
                ...
                <input
                  v-model.number="options.borderThickness"
                  type="number"
                  min="0"
                  max="60"
                  class="slider-num"
            ...
            <div v-if="overlayConfigs.length > 0" class="slider">
              ...
              <div class="overlay-config-checkboxes">
                <label
                  v-for="cfg in overlayConfigs"
                  :key="cfg.id"
                  class="checkbox-label overlay-config-item"
                >
                  <input
                    type="checkbox"
        ...
        <div class="acc-actions">
          <button class="btn-primary" :disabled="loading" @click="() => doPreview()">
            Preview
          </button>
    ...
    <div class="season-panel">
      <div class="season-header">
        ...
        <div class="season-controls">
          ...
          <button @click="deselectAllSeasons" class="season-ctrl-btn" title="Deselect All">
            None
          </button>
        </div>
      </div>
      ...
      <div class="season-list">
        ...
        >
          <div class="season-thumb-wrap">
            <img v-if="season.thumb" :src="season.thumb" :alt="season.title" class="season-thumb" />
      ...
      <div class="preview-inner">
        <div class="preview-existing">
          ...
          <div class="preview-label" style="margin-top: 14px">
            ...
            <button
              class="refresh-btn"
              title="Fetch logo from Plex"
              @click="fetchExistingLogo(true)"
            >
              <svg
                width="14"
                height="14"
          ...
          <div class="preview-main">
            <div class="preview-label">
              ...
              <div class="preview-actions float-right">
                ...
                <button
                  title="Send Logo to Plex"
                  class="btn-send-logo btn-inline"
                  :disabled="logoSending || !logoUrl"
                  @click="doSendLogoOnly"
                >
                  <svg
                    v-if="logoSending"
                    width="12"
                ...
                <button
                  ...
                  @click="doSave"
                >
                  💾 <span class="btn-label">Save to Disk</span>
                </button>
                <button
                  title="Send to Plex"
                  class="btn-plex btn-inline"
                  :disabled="loading"
                  @click="doSend"
                >
                  📺 <span class="btn-label">Send to Plex</span>
                </button>
              </div>
            </div>
            <div class="preview-container">
              ...
              <div
                v-if="isUniformLogo && showBoundingBox && lastPreview"
                class="bounding-box"
...
.upload-reselect:hover,
.upload-replace:hover {
  background: rgba(61, 214, 183, 0.35);
}
.upload-reselect.active {
  border-color: #3dd6b7;
  color: #3dd6b7;
}
.upload-remove {
