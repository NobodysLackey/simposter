...
function deriveValues(editor: HTMLElement, controls: LogoControls): { size: number; x: number; y: number } {
  ...
  return {
    // The universal renderer interprets logoScale as a fraction of poster width.
    // A simple-control value of 100 maps to a comfortable 70% visible width.
...
function applyValues(
  ...
  if (controls.mode === 'uniform') {
    // Canonical safe area: visible artwork is auto-trimmed by the renderer, so a
    // size of 100 now means the same thing regardless of the source PNG canvas.
...
function createPlacementPanel(editor: HTMLElement, controls: LogoControls): HTMLElement {
  ...
  const rows = controls.mode === 'uniform' ? [0, 1, 2] : [0, 1, 2]
  rows.forEach((row) => {
  ...
  // Canonicalize the existing preset once. This preserves its effective visual size
  // while removing the confusing interaction between scale and two independent boxes.
...
function installStyles(): void {
  ...
  style.textContent = `
    ...
    .logo-placement-grid.vertical-only .logo-placement-cell:nth-child(1) { grid-column: 2; }
    .logo-placement-grid.vertical-only .logo-placement-cell:nth-child(2) { grid-column: 2; }
