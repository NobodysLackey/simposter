...
def apply_save_location_variables(
    ...
    """
    Replace save-location variables with their actual values.

    TV posters are saved inside the existing show directory:
      show.jpg
      Season01.jpg
      Season02.jpg
    """
    ...
    # For television, {title} remains the existing show-directory name.
    # The actual show/season filename is selected later in api_save().
    ...
    if is_tv_show:
        ...
        result = result.replace("{season}", season_name)
    else:
        result = result.replace(
            "{season}",
...
def api_save(req: SaveRequest):
    ...
    if candidate.suffix:
        # The save template supplied a complete filename.
        base_dir = candidate.parent
        filename = candidate.name
    else:
        # The save template supplied a directory.
        base_dir = candidate

        if req.is_tv:
            filename = (
                f"Season{req.season_index:02d}.jpg"
